        ...
        {posts.slice(currentPage * 9, currentPage * 9 + 9).map(post => (
          <div key={post.slug}>
            <Link href={`/blog/${post.slug}`} className="block h-full rounded-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#c93820]">
              <article className="group bg-[#faf8f5] rounded-lg overflow-hidden border border-[#d9d6d0] hover:border-[#aaa79e] transition-colors duration-300 h-full flex flex-col">
                <div className="relative aspect-video overflow-hidden bg-[#e7e2da] shrink-0">
                  <Image src={getBlogCover(post.slug, post.cover_image)} alt={post.title} fill sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 33vw" className="object-cover transition-transform duration-700 motion-safe:group-hover:scale-[1.03]" />
                </div>
