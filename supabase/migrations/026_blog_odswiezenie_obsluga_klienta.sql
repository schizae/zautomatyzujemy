-- Odświeżenie artykułu automatyzacja-obslugi-klienta-ai-rag-msp
-- Źródło: docs/seo/odswiezenia/automatyzacja-obslugi-klienta.md, złożone skryptem scripts/seo/migracja-odswiezenia.mjs.
--
-- Treść jest podmieniana tylko wtedy, gdy w bazie stoi dokładnie ta wersja, z której składano
-- migrację (skrót md5 1a2ca56039438fc22ba7fbf6f5071fdf). Ponowne uruchomienie po udanym nic nie robi.

DO $$
DECLARE
  obecny_skrot TEXT;
BEGIN
  SELECT md5(content) INTO obecny_skrot FROM posts WHERE slug = 'automatyzacja-obslugi-klienta-ai-rag-msp';

  IF obecny_skrot IS NULL THEN
    RAISE EXCEPTION 'Brak artykułu automatyzacja-obslugi-klienta-ai-rag-msp';
  END IF;

  IF obecny_skrot = 'dad11e54abfcd34c845a8cd63a74867a' THEN
    RAISE NOTICE 'Artykuł jest już odświeżony';
    RETURN;
  END IF;

  IF obecny_skrot <> '1a2ca56039438fc22ba7fbf6f5071fdf' THEN
    RAISE EXCEPTION 'Treść artykułu zmieniła się od złożenia migracji, złóż ją ponownie';
  END IF;

  UPDATE posts SET
    title = $tresc$Automatyzacja obsługi klienta z AI: co oddać automatowi, a co zostawić ludziom$tresc$,
    excerpt = $tresc$Automatyzacja obsługi klienta krok po kroku: które pytania oddać AI, jak zbudować przekazanie do człowieka, co mówi AI Act o chatbotach i co mierzyć po starcie.$tresc$,
    target_keyword = $tresc$automatyzacja obsługi klienta$tresc$,
    ai_model = 'claude-opus-5',
    reviewed_at = now(),
    -- Git na Windowsie potrafi zamienić końce linii w pliku migracji, a skrót liczymy z LF.
    content = replace($tresc$Automatyzacja obsługi klienta ma sens tam, gdzie pytania się powtarzają, a odpowiedź da się oprzeć na dokumencie albo danych z systemu: status zamówienia, warunki zwrotu, godziny pracy, cennik. Reklamacje, sprawy sporne i klienci, którzy wprost proszą o człowieka, zostają przy ludziach, a zadaniem automatu jest szybko ich do nich przekazać.
...
-- Weryfikacja: tytuł nowy i skrót równy dad11e54abfcd34c845a8cd63a74867a.
--
--   SELECT title, target_keyword, md5(content) FROM posts WHERE slug = 'automatyzacja-obslugi-klienta-ai-rag-msp';
