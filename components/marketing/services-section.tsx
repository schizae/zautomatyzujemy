    ...
    <div className="mx-auto max-w-[1680px]">
      <div className="grid items-start gap-12 lg:grid-cols-[0.85fr_1.15fr]">
        <div>
          ...
          <h2 className="font-body text-5xl font-semibold leading-[1.04] tracking-[-0.055em] xl:text-[68px]">Mniej przeszkód.<br />Więcej <span className="font-editorial font-normal italic tracking-[-0.06em]">działania.</span></h2>
          <p className="my-8 text-lg text-[#dedbd5]">Wybierz obszar, który chcesz usprawnić.</p>
        <div className="min-w-0">{services.map((item, i) => <Button key={item.name} onClick={() => setSelected(i)} aria-pressed={selected === i} aria-controls="service-detail" className={`flex h-auto w-full justify-between whitespace-normal rounded-none border-0 border-b border-white/20 bg-transparent px-0 py-5 text-left text-lg hover:bg-white/5 md:text-2xl ${selected === i ? 'text-[#f34c30]' : 'text-[#dedbd5]'}`}>
          <span>
        ...
        <div id="service-detail" className="relative isolate min-w-0 overflow-hidden rounded-xl border border-[#d9d6d0] bg-[#f5f2ed] min-h-[570px] p-6 text-[#151719] md:p-8 xl:p-10">
          ...
          <AnimatePresence mode="wait">
            <motion.div key={selected} initial={{ opacity: 0, y: reduced ? 0 : 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: reduced ? 0 : .18 }}>
              ...
              {selected === 0 ? <div className="grid items-center gap-4 sm:grid-cols-[1fr_auto_1fr]">
                <div className="min-h-64 rounded-lg border border-[#d0ccc5] bg-[#fffdfa] p-5 shadow-[0_12px_32px_-16px_rgba(21,23,25,0.35)]"><div className="mb-5 flex items-center gap-3 border-b border-black/10 pb-4 text-sm"><Mail className="size-5" />Nowa wiadomość</div><p className="text-xs text-[#62625d]">Od: klient@przyklad.pl</p><p className="mt-2 text-xs text-[#62625d]">Temat: Zapytanie o ofertę</p><p className="mt-6 leading-relaxed">Proszę o ofertę montażu klimatyzacji.</p></div>
                <ArrowUpRight className="mx-auto size-6" />
                <div className="min-h-64 rounded-lg border border-[#d0ccc5] bg-[#fffdfa] p-5 shadow-[0_12px_32px_-16px_rgba(21,23,25,0.35)]"><div className="mb-5 flex items-center gap-3 text-sm font-semibold"><FileText className="size-5" />Nowe zgłoszenie</div><dl className="space-y-4 text-sm"><div><dt className="text-xs text-[#62625d]">Temat</dt><dd>Montaż klimatyzacji</dd></div><div><dt className="text-xs text-[#62625d]">Źródło</dt><dd>E-mail</dd></div><div><dt className="text-xs text-[#62625d]">Status</dt><dd className="text-[#c93820]">Nowe</dd></div></dl><p className="mt-5 rounded-md border border-[#eea38f] bg-[#f8d4c9] p-3 text-sm">✓ Szkic odpowiedzi gotowy</p></div>
              </div> : <><p className="my-5 leading-relaxed text-[#62625d]">{service.description}</p><ol className="space-y-2">{service.steps.map((step, i) => <li key={step}><div className="flex items-center gap-3 rounded-lg border border-[#d9d6d0] bg-white/60 p-4 text-sm"><Check className="size-4 text-[#c93820]" />{step}</div>{i < 2 && <ArrowDown className="mx-auto mt-2 size-4 text-[#62625d]" />}</li>)}</ol></>}
          ...
          <Button asChild className="mt-8 h-auto rounded-md bg-[#c93820] px-6 py-4 text-white hover:bg-[#c93820]"><Link href={selected === 0 ? "/#klara" : "/#kontakt"}>{selected === 0 ? "Wypróbuj Klarę →" : "Porozmawiajmy o tym →"}</Link></Button>
        </div>
      </div>
